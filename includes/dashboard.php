...
<title><?php echo(User::getInstance()->getName()); ?> - {app_name/}</title>
<link
	rel="alternate" type="application/atom+xml"
	title="<?php echo(User::getInstance()->getName()); ?> - {app_name/}
	(Atom Feed)"
	href="<domain/>
	/util/atom.php?a=<?php echo(User::getInstance()->getAid()); ?>">
</head>
<body onload="init()">
	<input id="user" type="hidden" name="user"
		value="<?php echo(User::getInstance()->getName()); ?>" />
	<input id="email" type="hidden" name="email"
		value="<?php echo(User::getInstance()->getEmail()); ?>" />
	<input id="token" type="hidden" name="token"
		value="<?php echo(User::getInstance()->getToken()); ?>" />
	<div id="dashboard_top">
		<div class="hc">
			<a href="<?php echo(DOMAIN.'/util/settings.php');?>"
				...
				{fr}Paramètres{/fr} {es}Configuración{/es} {/i18n}"><img id="config"
				class="help" src="graphics/config.png" align="top" border="0"
				width="42" height="42" />
				<div id="help_config" class="docu"
					style="width: 220px; height: 44px;">
					{i18n key="tab20"} {en} <a
						href="javascript:tubeTutorial('akZ90qEgKEQ')">Change character and
						settings</a> for the user account.{/en} {de} <a
		...
		<div class="hc">
			<input id="timeText" class="input_text help" type="text" size="14"
				...
				onkeyup="checkTimeText(event, this.value)"
				value="{i18n
				 ...
				 ref='tab43' />" /> <img id="closeTimeEditor"
				onclick="Timeline.setCursor(null);" src="graphics/close.png"
				title="{i18n key='tab8'}
    			{en}close time editing{/en}
    			{de}Zeiteditierung Beenden{/de}
    			{fr}finir édition des temps{/fr}
    			{es}final edición del tiempo{/es}
			  {/i18n}" style="display:none;" />
		...
		<div class="hc">
			...
			maxlength="120" />
			<div id="help_info" class="docu" style="width: 280px; height: 134px;">
				...
				every ± 2 minutes, otherwise the existing info will be modified. By
				entering a blank info an existing info is removed.{/en} {de}Infos
				zur Zeitleiste hinzufügen, ändern und wieder entfernen. Es sind bis
				zu 120 Zeichen pro Info möglich, durch die Return-Taste wird eine
				Info eingetragen. Eine neue Info kann nur alle ± 2 Minuten
				eingetragen werden, ansonsten wird die existierende Info geändert.
				Durch Eintragen einer leeren Info kann eine existierende Info wieder
				entfernt werden.{/de} {fr}Informations sur la timeline pour ajouter,
				modifier et supprimer. Il ya jusqu'à 120 caractères par les
				informations disponibles, la touche de retour est une base de
				...
				información disponible, la tecla de retorno es una base de datos de
				información. Una nueva información se puede introducir sólo una vez
				cada ± 2 minutos, de lo contrario la información existente
				modificada. Al entrar en un espacio en blanco que la información es
				una información existente se quita.{/es}{/i18n}
			</div>
		</div>
		<form name="report" action="<?php echo(DOMAIN.'/util/report.php'); ?>"
			method="GET" onsubmit="return updateReportTime();" target="_blank">
			<div class="hc">
		...
		<div id="doingHolder">
			<div id="doing" title="{i18n ref='bot4'/}" style="display: none;"></div>
			<div id="status" class="status"></div>
		</div>
		<img id="showHelp" src="graphics/help.png"
			style="vertical-align: bottom; cursor: help;"
			onclick="toggleShowHelp()" title="{i18n         ref='bot0' />"
		border="0" width="24" height="24" /> </span><span> <a id="feedLink"
			href="<?php echo(DOMAIN."/util/feedform.php"); ?>"
			title="{i18n



			 ref='fdf0'}{/i18n}" onclick="return showAbove('feedback',
		...
		<div class="hc">
			...
			<div id="help_doLogout" class="docu"
				...
				schließen.{/de} {fr}Terminer la session courante.{/fr} {es}Final de
				la sesión actual.{/es}{/i18n}
			</div>
		</div>
		</form>
	</div>
			...
			<ul>
			 ...
			 ?>
				<li style="width: 16px; height: 36px;"><span
					...
					href="util/ajax.php"></a>
				</li>
			</ul>
			<div id="fields" class="help"></div>
			<div id="help_fields" class="docu"
				style="width: 320px; height: 314px;">
				{i18n key='tab12'} {en} <a
					href="javascript:tubeTutorial('gP9O8lLCleU')">Change distribution,
					description, reportables and color of the fields</a>.{/en} {de} <a
					href="javascript:tubeTutorial('hk05wFpoRyM')">Aufteilung,
					Bezeichnung, Berichtbarkeit und Farbe der Felder ändern</a>.{/de} {fr}
				<a href="javascript:tubeTutorial('gP9O8lLCleU')">Changement de
					distribution, description, rapportables et la couleur des champs</a>.{/fr}
				{es} <a href="javascript:tubeTutorial('gP9O8lLCleU')">Distribución,
				...
				{i18n key="tab18"} {en}Editing is possible by moving the mouse over
				the fields.{/en} {de}Zum Editieren die Maus über die Felder bewegen.{/de}
				{fr}Déplacer la souris sur les champs pour l'édition.{/fr} {es}Mover
				el ratón sobre los campos de edición.{/es}{/i18n}
			</div>
		</div>
	</div>
	<div id="timeline" class="timeline">
		<noscript>
			<div
				style="text-align: center; font-size: 20px; color: red; text-decoration: blink;">
				{i18n ref='tab56' /}
			</div>
		</noscript>
	</div>
