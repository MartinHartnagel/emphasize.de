    ...
    .extend({
      createUploadIframe : function(id, uri) {
        ...
        if (window.ActiveXObject) {
          io = Object(document.createElement('<iframe id="' + frameId
              + '" name="' + frameId + '" />'));
          ...
          io = Object(document.createElement('iframe'));
          io.id = frameId;
          io.name = frameId;
        }
      ...
      uploadHttpData : function(r, type) {
        ...
        var ds = dataparsed[0].replace(/&lt;/g, "<").replace(/&gt;/g, ">")
            .replace(/&amp;/g, "&");
        data = type == "xml" || "{ " + ds + " }"; // added by Jude
        // If the type is "script", eval it in global context
        if (type == "script")
          $.globalEval(data);
