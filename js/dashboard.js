...
Dashboard = function() {
    ...
    if ((td != null) && (td.logGrid_y == tdo.logGrid_y)
      ...
      if (record) {
        if (actions.length > 20)
          actions.pop();
        actions.push("if (" + self + ".isValid())  " + self + ".verticalMerge("
            + self + ".getTd(" + tdo.logGrid_x + "," + tdo.logGrid_y + "));");
      }
      // invalidateGrid
      grid = undefined;
    ...
    if ((td != null) && (td.logGrid_x == tdo.logGrid_x)
      ...
      if (record) {
        ...
        actions.push("if (" + self + ".isValid()) " + self
            + ".horizontalMerge(" + self + ".getTd(" + tdo.logGrid_x + ","
            + tdo.logGrid_y + "));");
      }
      // invalidateGrid
      grid = undefined;
  ...
  Dashboard.prototype.verticalSplit = function(element) {
    ...
    if (record) {
      if (actions.length > 20)
        actions.pop();
      actions.push("if (" + self + ".isValid()) " + self + ".verticalSplit("
          + self + ".getTd(" + tdo.logGrid_x + "," + tdo.logGrid_y + "));");
    }
    var td;
    if (tdo.colSpan > 1) {
  ...
  function cloneText(tdo) {
    hideEdits(over);
    var text = Dashboard.prototype.getEntry(tdo).replace(/&lt;/g, "<").replace(
        /&gt;/g, ">").replace(/ \((\d+)\)$/, "");
    var c = 2;
  ...
  Dashboard.prototype.horizontalSplit = function(element) {
    ...
    if (record) {
      if (actions.length > 20)
        actions.pop();
      actions.push("if (" + self + ".isValid()) " + self + ".horizontalSplit("
          + self + ".getTd(" + tdo.logGrid_x + "," + tdo.logGrid_y + "));");
    }
    var td;
    if (tdo.rowSpan > 1) {
  ...
  Dashboard.prototype.editLink = function(element) {
    ...
    var doChange = function() {
      ...
      if (link == "") {
        ...
        $(tdo).html('<a href="' + link + '" target="_blank">' + text + '</a>');
      }
      notify("edit");
      hideEdits(over);
    };
    ...
    if (over != tdo) {
      ...
      $(
          editControl(
              ...
              "left:" + (-hs.w / 2 - 8) + "px;top:50%;margin-top:"
                  + (-hs.h / 2) + "px;width:" + hs.w + "px;height:" + hs.h
                  + "px;",
              function(event) {
                event.stopPropagation();
      ...
      $(
          editControl(
              ...
              "left:50%;margin-left:" + (-vs.w / 2) + "px;top:"
                  + (-vs.h / 2 - 8) + "px;width:" + vs.w + "px;height:" + vs.h
                  + "px;",
              function(event) {
                event.stopPropagation();
      ...
      if (canDoHorizontalGlue(tdo)) {
        $(
            editControl(
                ...
                "left:50%;margin-left:" + (-8) + "px;bottom:" + (-hg.h / 2 - 4)
                    + "px;width:" + hg.w + "px;height:" + hg.h + "px;",
                function(event) {
                  event.stopPropagation();
      ...
      if (canDoVerticalGlue(tdo)) {
        $(
            editControl(
                ...
                "right:" + (-vg.w / 2) + "px;top:50%;margin-top:" + (-8)
                    + "px;width:" + vg.w + "px;height:" + vg.h + "px;",
                function(event) {
                  event.stopPropagation();
